...
/**
 * Render state for a BBS model-form thumbnail drawn as a vanilla special GUI element — the same mechanism
 ...
 * back into {@link ModelFormRenderer#renderUIPreview} to render the form's model off-screen, and the deferred
 * GUI composites it into the cell. This indirection is needed because the list draws each cell in the GUI
 * record phase, where a direct immediate 3D draw cannot composite (two-phase GUI).
 *
...
{
    public BbsFormGuiElementRenderState(ModelFormRenderer renderer, float angle, float transition, Matrix3x2f pose, int x1, int y1, int x2, int y2, float scale, @Nullable ScreenRect scissorArea)
    {
